...
async function getDownloads() {
  try {
    return JSON.parse(
      await fs.readFile(STATS_FILE, "utf8")
    ).downloads || 0;
  } catch {
    await fs.writeFile(
      STATS_FILE,
...
app.post("/api/convert", async (req, res) => {
  ...
  // Cada download fica em uma pasta própria
  ...
  // Nome temporário usado pelo yt-dlp
  const output = path.join(
    jobDir,
    "video.%(ext)s"
  );

  // Depois da conversão, esperamos este arquivo
  ...
  // Responde imediatamente para o site
  res.json({ id });
  ...
  try {
    await ytdlp(url, {
      ...
      // Usa o Deno instalado no PC
      jsRuntimes: "deno",

      // Permite baixar o EJS necessário
      // para resolver os desafios atuais
      // do YouTube
  ...
  async (req, res) => {
    ...
    res.download(
      ...
      async (err) => {
        ...
        // Se o download terminou sem erro,
        // aumenta o contador
        if (!err) {
